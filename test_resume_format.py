...
# Test the actual resume format from the JSON
...
print("=== TESTING RESUME FORMAT ===")
print("Resume text:")
print(resume_text[:200] + "...")
print()

# Test current pattern from file
current_pattern = re.compile(r'^([^:]+):\s*([^:]+):\s*[^-\n]+-\s*[^-\n]+', re.MULTILINE)
current_matches = current_pattern.findall(resume_text)
print(f"Current pattern matches: {current_matches}")

# Test correct pattern for this resume format
# Format: "Company: Date Range (Location: City, State)"
correct_pattern = re.compile(r'^([^:]+):\s*[^-]+-\s*[^-\n]+', re.MULTILINE)
correct_matches = correct_pattern.findall(resume_text)
print(f"Correct pattern matches: {correct_matches}")

# Test splitting by company names
split_pattern = re.compile(r'\n(?=[A-Z][a-zA-Z\s&]+:\s*[A-Z][a-z]+\s*\d{4})')
split_parts = split_pattern.split(resume_text)
print(f"Split parts count: {len(split_parts)}")
for i, part in enumerate(split_parts):
    if part.strip():
        company = part.strip().split(':')[0]
...
for i, line in enumerate(lines[:10]):
    ...
    if ':' in line and '-' in line and any(month in line for month in ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']):
        print(f"  -> JOB HEADER LINE DETECTED!")
