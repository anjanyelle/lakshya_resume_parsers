...
# ============================================================================
# Create Lakshya Colab Training Package
# ============================================================================

set -e  # Exit on error

echo "🚀 Creating Lakshya Colab Training Package..."
echo ""

# Define paths
PROJECT_ROOT="/Users/anjanyelle/Desktop/untitled folder 3/Lakshya-LLM-Resume-Parser"
PACKAGE_NAME="Lakshya-Colab-Training"
...
mkdir -p "${PACKAGE_DIR}/ai-service/training/data"
mkdir -p "${PACKAGE_DIR}/ai-service/models"

# Copy training script
echo "📄 Copying training script..."
cp "${PROJECT_ROOT}/ai-service/training/train_colab_standalone.py" \
   "${PACKAGE_DIR}/ai-service/training/"

# Copy training data
echo "📊 Copying training data..."
cp "${PROJECT_ROOT}/ai-service/training/data/simple_dataset_train.conll" \
   "${PACKAGE_DIR}/ai-service/training/data/"
cp "${PROJECT_ROOT}/ai-service/training/data/simple_dataset_test.conll" \
   "${PACKAGE_DIR}/ai-service/training/data/"
...
═══════════════════════════════════════════════════════════════
  LAKSHYA RESUME NER - GOOGLE COLAB QUICK START
═══════════════════════════════════════════════════════════════
...
Then select: Lakshya-Colab-Training.zip
...
🔧 STEP 3: INSTALL DEPENDENCIES
──────────────────────────────────────────────────────────────
    !pip install -q transformers==4.44.0 datasets==2.19.0 \
                    accelerate==0.33.0 evaluate==0.4.1 \
                    seqeval scikit-learn torch==2.4.0
...
⏱️  Training takes ~60-70 minutes on T4 GPU
...
✅ DONE!
──────────────────────────────────────────────────────────────
Your trained model is now downloaded!
...
cd "${PROJECT_ROOT}"
zip -r "${ZIP_FILE}" "${PACKAGE_NAME}" -x "*.DS_Store" -q
...
echo ""
echo "📊 Contents:"
echo "   • Training script: train_colab_standalone.py"
echo "   • Training data: 15,433 sentences"
echo "   • Test data: 1,930 sentences"
echo "   • Documentation: README.md + QUICK_START.txt"
echo ""
echo "🚀 Next Steps:"
echo "   1. Upload ${PACKAGE_NAME}.zip to Google Colab"
echo "   2. Follow instructions in QUICK_START.txt"
echo "   3. Train for ~60-70 minutes on T4 GPU"
echo ""
echo "🎯 Expected F1 Score: ~96%"
echo ""
