#!/usr/bin/env python3
"""
Test the 'other' section resolution specifically
"""
...
def test_other_section_resolution():
    """Test how the 'other' section is being resolved"""
...
Visualization"""
    ...
    print("Testing 'other' section resolution...")
    print(f"Content length: {len(other_content)} chars")
    ...
    # Test resolution
    result = validator.resolve_unknown_section(other_content)
    print(f"Resolution result: {result}")
    
    # Check education fingerprint
    edu_fingerprint = validator.get_expected_fingerprint('education')
    print(f"Education fingerprint: {edu_fingerprint}")
    
    # Check if education conditions are met
    edu_conditions_met = (
        profile['DEGREE'] >= 2 and 
        profile['ORG'] <= 10 and 
        len(other_content) < 2000
    )
    print(f"Education conditions met: {edu_conditions_met}")
    print(f"  DEGREE >= 2: {profile['DEGREE'] >= 2}")
    print(f"  ORG <= 10: {profile['ORG'] <= 10}")
    print(f"  Length < 2000: {len(other_content) < 2000}")

if __name__ == "__main__":
    test_other_section_resolution()
