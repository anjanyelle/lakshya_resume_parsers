...
export const analyticsService = {
  ...
  async getParsingStats(): Promise<ParsingStats> {
    const response = await api.get('/api/v1/analytics/parsing-stats');
    return response.data;
  },

  // Get skill distribution
  async getSkillDistribution(): Promise<SkillDistribution[]> {
    const response = await api.get('/api/v1/analytics/skill-distribution');
    return response.data;
  },

  // Get accuracy overview
  async getAccuracyOverview(): Promise<AccuracyOverview> {
    const response = await api.get('/api/v1/accuracy/overview');
    return response.data;
  },
  ...
  async getUploadTrends(dateRange: string = '30'): Promise<UploadTrend[]> {
    const response = await api.get(`/api/v1/analytics/upload-trends?range=${dateRange}`);
    return response.data;
  },

  // Get recruiter activity
  async getRecruiterActivity(): Promise<RecruiterActivity> {
    const response = await api.get('/api/v1/analytics/recruiter-activity');
    return response.data;
  },

  // Export analytics as CSV
  async exportAnalyticsCSV(dateRange: string): Promise<Blob> {
    const response = await api.get(`/api/v1/analytics/export/csv?range=${dateRange}`, {
      responseType: 'blob',
    });
    return response.data;
  },
  ...
  async exportAnalyticsPDF(dateRange: string): Promise<Blob> {
    const response = await api.get(`/api/v1/analytics/export/pdf?range=${dateRange}`, {
      responseType: 'blob',
    });
    return response.data;
  },
