...
class SocketService {
  ...
  private setupEventListeners() {
    ...
    this.socket.on('parsing:progress', (data: { candidateId: string; progress: number; message: string }) => {
    ...
    this.socket.on('parsing:complete', (data: { candidateId: string; data: any }) => {
    ...
    this.socket.on('parsing:failed', (data: { candidateId: string; error: string }) => {
    ...
    this.socket.on('matching:progress', (data: { jobId: string; progress: number; message: string }) => {
    ...
    this.socket.on('matching:complete', (data: { jobId: string; results: any[] }) => {
  ...
  subscribeToParsingProgress(callback: (data: { candidateId: string; progress: number; message: string }) => void) {
    if (this.socket) {
  ...
  subscribeToParsingComplete(callback: (data: { candidateId: string; data: any }) => void) {
    if (this.socket) {
  ...
  subscribeToParsingFailed(callback: (data: { candidateId: string; error: string }) => void) {
    if (this.socket) {
  ...
  subscribeToMatchingProgress(callback: (data: { jobId: string; progress: number; message: string }) => void) {
    if (this.socket) {
  ...
  subscribeToMatchingComplete(callback: (data: { jobId: string; results: any[] }) => void) {
    if (this.socket) {
...
export const connectSocket = (userId?: string) => socketService.connectSocket(userId)
export const disconnectSocket = () => socketService.disconnectSocket()
export const subscribeToParsingProgress = (callback: (data: { candidateId: string; progress: number; message: string }) => void) => 
  socketService.subscribeToParsingProgress(callback)
export const subscribeToParsingComplete = (callback: (data: { candidateId: string; data: any }) => void) => 
  socketService.subscribeToParsingComplete(callback)
export const subscribeToParsingFailed = (callback: (data: { candidateId: string; error: string }) => void) => 
  socketService.subscribeToParsingFailed(callback)
export const subscribeToMatchingProgress = (callback: (data: { jobId: string; progress: number; message: string }) => void) => 
  socketService.subscribeToMatchingProgress(callback)
export const subscribeToMatchingComplete = (callback: (data: { jobId: string; results: any[] }) => void) => 
