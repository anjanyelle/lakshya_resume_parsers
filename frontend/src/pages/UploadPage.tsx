...
import { connectSocket, subscribeToParsingProgress, subscribeToParsingComplete, subscribeToParsingFailed } from '../services/socket'
...
export default function UploadPage() {
  ...
  useEffect(() => {
    ...
    const handleProgress = (data: { candidateId: string; progress: number; message: string }) => {
  ...
  const handleUpload = async (uploadFile: UploadFile) => {
    try {
      ...
      setUploadFiles(prev => prev.map(f => 
        f.id === uploadFile.id 
          ? { ...f, status: 'uploading', message: 'Uploading...', progress: 0 }
          : f
      ))
      ...
      setUploadFiles(prev => prev.map(f => 
        f.id === uploadFile.id 
          ? { ...f, candidateId: candidate.id, status: 'parsing', message: 'Extracting text...', progress: 25 }
          : f
      ))
  ...
  const getConfidenceColor = (score: number) => {
    if (score >= 0.80) return 'text-green-600 bg-green-100'
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Upload resumes to parse candidate information using AI</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">{uploadFiles[0].file.name} ({formatFileSize(uploadFiles[0].file.size)})</p>
            </div>
      ...
      {currentUpload && !isBulkMode && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="mb-4">
            <h3 className="text-lg font-medium text-gray-900">Uploading: {currentUpload.file.name}</h3>
            <p className="text-sm text-gray-500">Size: {formatFileSize(currentUpload.file.size)}</p>
          </div>

          {/* Progress Bar */}
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">{currentUpload.message}</span>
              <span className="text-sm text-gray-500">{currentUpload.progress}%</span>
            </div>
          ...
          {currentUpload.status === 'completed' && currentUpload.result && (
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h4 className="text-green-800 font-medium mb-2">✅ Parsing Complete!</h4>
              </div>
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{currentUpload.result.name || 'Not found'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Email</p>
                    <p className="font-medium">{currentUpload.result.email || 'Not found'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Phone</p>
                    <p className="font-medium">{currentUpload.result.phone || 'Not found'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Experience</p>
                    <p className="font-medium">{currentUpload.result.work_experience?.length || 0} positions</p>
                  </div>
                </div>

                {/* Skills */}
                {currentUpload.result.skills && currentUpload.result.skills.length > 0 && (
                  <div className="mt-4">
                    <p className="text-sm text-gray-600 mb-2">Top Skills</p>
                    <div className="flex flex-wrap gap-2">
                      {currentUpload.result.skills.slice(0, 5).map((skill: string, index: number) => (
                ...
                <div className="mt-4">
                  ...
                  <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getConfidenceColor(currentUpload.result.confidence?.overall || 0)}`}>
                ...
                <div className="mt-6 flex space-x-3">
                  <button
                    onClick={() => navigate(`/candidates/${currentUpload.candidateId}`)}
                    className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors"
                  >
                    View Full Profile
                  </button>
      ...
      {isBulkMode && uploadFiles.length > 0 && (
        <div className="space-y-4">
          ...
          {uploadFiles.map((uploadFile) => (
            <div key={uploadFile.id} className="bg-white rounded-lg shadow-sm p-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`px-2 py-1 text-xs font-medium rounded ${
                    ...
                    uploadFile.status === 'parsing' ? 'bg-yellow-100 text-yellow-800' :
              ...
              {uploadFile.status !== 'pending' && (
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-gray-600">{uploadFile.message}</span>
                    <span className="text-sm text-gray-500">{uploadFile.progress}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1">
                    <div 
                      className={`h-1 rounded-full transition-all duration-300 ${
                        uploadFile.status === 'error' ? 'bg-red-600' : 'bg-indigo-600'
                      }`}
          ...
          {/* Summary */}
          {uploadFiles.length > 0 && uploadFiles.every(f => f.status === 'completed' || f.status === 'error') && (
            <div className="bg-white rounded-lg shadow-sm p-6">
