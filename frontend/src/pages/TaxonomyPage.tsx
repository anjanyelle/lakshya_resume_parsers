  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Taxonomy manager</h1>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add entry">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input value={addName} onChange={(event) => setAddName(event.target.value)} placeholder="Name" />
          </div>
          ...
          {addType === 'skill' && (
            <>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600">Synonyms</label>
                <Input
                  value={addSynonyms}
              ...
              <div className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  >
                      ...
                      <span className="text-slate-500">{skill.synonyms ?? '—'}</span>
                    </div>
                  ))
                )}
              </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-subtle">
            ...
            <div className="mt-3 space-y-3 text-sm text-slate-700">
              {(degrees.length ? degrees : [{ name: 'Loading...' }]).map((degree) => (
                <div key={degree.name} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-subtle">
            <h3 className="text-sm font-semibold text-slate-900">Universities</h3>
            <div className="mt-3 space-y-3 text-sm text-slate-700">
              {(universities.length ? universities : [{ name: 'Loading...' }]).map((uni) => (
                <div key={uni.name} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 p-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-subtle">
            ...
            <div className="mt-3 space-y-3 text-sm text-slate-700">
              {(certifications.length ? certifications : [{ name: 'Loading...' }]).map(
                (cert) => (
                <div key={cert.name} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 p-3">
