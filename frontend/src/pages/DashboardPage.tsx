...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const avgScore = matchResults.length > 0 
      ? Math.round(matchResults.reduce((acc, match) => acc + match.overall_score, 0) / matchResults.length)
      : 0
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Welcome back! Here's what's happening with your resume parsing today.</p>
      </div>
                    ...
                    <tr key={candidate.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`
                          ...
                          ${candidate.parsing_status?.status === 'completed' ? 'bg-green-100 text-green-800' :
                            candidate.parsing_status?.status === 'processing' ? 'bg-yellow-100 text-yellow-800' :
                            candidate.parsing_status?.status === 'failed' ? 'bg-red-100 text-red-800' :
                  ...
                  <div key={match.id} className="px-6 py-4 hover:bg-gray-50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div className="ml-3">
                          <p className="text-sm font-medium text-gray-900">{match.candidate_name}</p>
                          <p className="text-xs text-gray-500">{match.candidate_email}</p>
                        </div>
                      </div>
