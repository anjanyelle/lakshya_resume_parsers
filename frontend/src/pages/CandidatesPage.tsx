...
export default function CandidatesPage() {
  ...
  const filteredCandidates = candidates.filter(candidate => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      candidate.skills?.some(skill => skill.skill_name.toLowerCase().includes(searchLower))
          ...
          {paginatedCandidates.map((candidate) => (
            <div key={candidate.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-6">
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                {/* Confidence Badge */}
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${getConfidenceColor(candidate.parsing_status?.confidence_score || 0)}`}>
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-900">{getExperienceSummary(candidate.work_experience || [])}</p>
              </div>
