...
interface ApplicationContextValue {
  ...
  markResumeParsingComplete: (fileName: string, fileSizeKb: number) => void;
  nextStep: () => void;
  prevStep: () => void;
  saveDraft: () => void;
  loadDraft: () => void;
...
const mergeWithParsedResume = (source: ApplicationData): ApplicationData => {
...
export function ApplicationProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const saveSection = useCallback(<K extends keyof ApplicationData>(
    ...
    setApplication((prev) => ({ ...prev, [section]: value }));
  }, []);

  const markResumeParsingComplete = useCallback((fileName: string, fileSizeKb: number) => {
    setApplication((prev) =>
