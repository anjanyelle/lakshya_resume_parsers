...
export const useJobStore = create<JobState & JobActions>((set, get) => ({
  ...
  deleteJob: async (id: string) => {
    try {
      ...
      set((state) => ({
        ...
        matchResults: state.matchResults.filter(result => result.job_id !== id),
      }))
  ...
  runMatching: async (jobId: string, limit = 20) => {
    ...
    try {
      const response = await api.post(`/matching/job/${jobId}/candidates`, { limit })
      ...
      toast.success(`Matching completed for ${response.data.total_candidates} candidates`)
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || 'Failed to run matching'
  ...
  fetchMatchResults: async (jobId: string) => {
    set({ isLoading: true, error: null })
    try {
      const response = await api.get(`/matching/job/${jobId}/results`)
      set({ matchResults: response.data.matches, isLoading: false })
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || 'Failed to fetch match results'
      set({ error: errorMessage, isLoading: false })
      toast.error(errorMessage)
    }
  },
