...
export function getDisplayWorkHistory(
  ...
  // Check for complete_resume_json first (enhanced ML pipeline), then fallback to old format
  const workData = parsedData?.complete_resume_json?.work || parsedData?.work_experience
  let structured = workHistoryFromParsed(workData)
...
export function getDisplayEducation(
  ...
  // Check for complete_resume_json first (enhanced ML pipeline), then fallback to old format
  const educationData = parsedData?.complete_resume_json?.education || parsedData?.education
  const structured = educationFromParsed(educationData)
  if (structured.length > 0) return structured
...
export function getDisplayCertifications(
  ...
  // Check for complete_resume_json first (enhanced ML pipeline), then fallback to old format
  const certData = parsedData?.complete_resume_json?.certifications || parsedData?.certifications
  const structured = certificationsFromParsed(certData)
  if (structured.length > 0) return structured
  return []
}
...
export function shouldUseParsedDataFallback(
  ...
  // Check for complete_resume_json first (enhanced ML pipeline), then fallback to old format
  const completeJson = parsedData?.complete_resume_json || {}
  const workData = completeJson?.work || parsedData?.work_experience
  const educationData = completeJson?.education || parsedData?.education
  const certData = completeJson?.certifications || parsedData?.certifications
  const skillsData = completeJson?.skills || parsedData?.skills
  const contactData = completeJson?.basics || parsedData?.contact

  const hasParsedWork = Array.isArray(workData) && workData.length > 0
  const hasParsedEdu = Array.isArray(educationData) && educationData.length > 0
  const hasParsedCerts = Array.isArray(certData) && certData.length > 0
  const hasParsedSkills = Array.isArray(skillsData) && skillsData.length > 0
