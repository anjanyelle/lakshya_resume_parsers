...
export default function CompactCandidateRow({
  ...
  if (item) {
    ...
    return (
      <div className={`flex items-center gap-4 rounded-2xl border transition-all duration-300 p-3 shadow-sm animate-in fade-in slide-in-from-bottom-2 ${isError ? 'bg-rose-50/50 border-rose-100' : 'bg-white dark:bg-slate-800/50 border-slate-300 dark:border-slate-700/50'
        }`}>
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl border flex-shrink-0 transition-colors ${isError ? 'bg-rose-100 text-rose-500 border-rose-200' :
          isProcessing ? 'bg-brand-50 dark:bg-brand-900/30 text-brand-600 border-brand-100 dark:border-brand-900/20' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <h4 className="text-[13px] font-black text-[#1a2340] dark:text-slate-100 truncate pr-4">{item.file.name}</h4>
            <span className={`text-[10px] font-black uppercase tracking-widest ${isError ? 'text-rose-600' : 'text-slate-500 dark:text-slate-500'
              }`}>
        ...
        <div className="flex items-center gap-3 pr-2">
          <span className={`text-xs font-black ${isError ? 'text-rose-500' : 'text-brand-600 dark:text-brand-400'}`}>
  ...
  if (candidate) {
    ...
    return (
      <div className="flex items-center gap-4 rounded-xl border border-slate-300 bg-white p-3 shadow-premium hover:border-brand-200 hover:shadow-md transition-all duration-300 group animate-in fade-in slide-in-from-top-2">
        <div
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-[14px] font-black text-[#1a2340] dark:text-slate-100 truncate uppercase tracking-tight">{candidate.full_name || 'Anonymous'}</h4>
        ...
        <div className="flex items-center gap-6 pr-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-lg text-slate-400 hover:text-brand-600 hover:bg-brand-50 transition-all active:scale-90"
              title="View Details"
            >
