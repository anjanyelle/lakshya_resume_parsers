...
export default function PersonalDetailsSection({
  ...
  const fields = [
    { key: 'full_name', id: 'full_name', label: 'Candidate Name', value: candidate.full_name ?? '' },
    { key: 'email', id: 'email', label: 'Candidate Email', value: candidate.email ?? '' },
    { key: 'phone', id: 'phone', label: 'Candidate Phone', value: candidate.phone ?? '' },
    { key: 'location', id: 'location', label: 'Location', value: candidate.location ?? '' },
    { key: 'linkedin_url', id: 'linkedin_url', label: 'LinkedIn', value: candidate.linkedin_url ?? '' },
    { key: 'github_url', id: 'github_url', label: 'GitHub', value: candidate.github_url ?? '' },
  ]
  ...
  useScrollToField(panelScrollToFieldId, getFieldElement, onPanelScrollComplete)

  useEffect(() => {
    if (!autoEditFieldId || readOnly) return
    const value = (candidate as unknown as Record<string, unknown>)[autoEditFieldId]
  ...
  const saveField = async (key: string) => {
    ...
    const original = (candidate as unknown as Record<string, unknown>)[key] ?? ''
