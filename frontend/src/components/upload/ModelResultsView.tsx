...
export default function ModelResultsView({ modelResults }: ModelResultsViewProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-brand-50 to-purple-50 rounded-2xl p-6 border border-brand-200">
          ...
          {education && education.length > 0 ? (
            <div className="space-y-4">
              {education.map((edu: any, idx: number) => (
                <div key={idx} className="border-l-2 border-brand-300 pl-4 py-2">
                  <div className="space-y-1">
                    {edu.degree && (
                      <div className="flex gap-2">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {degrees.map((degree: string, idx: number) => (
                  <span key={idx} className="px-2 py-1 bg-brand-100 text-brand-700 text-xs rounded-md">
                    {degree}
                  </span>
                ))}
              </div>
