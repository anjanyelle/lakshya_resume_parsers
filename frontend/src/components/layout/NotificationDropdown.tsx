...
export default function NotificationDropdown({ onClose }: NotificationDropdownProps) {
  ...
  const typeStyles: Record<string, { bg: string; icon: any; color: string }> = {
    ...
    info: { bg: 'bg-orange-50 text-orange-500', icon: Bell, color: 'text-orange-400' },
      ...
      <div className="flex items-center justify-between border-b border-slate-100 bg-white/50 px-4 py-4">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 rounded-lg text-slate-400 hover:text-orange-500 hover:bg-orange-50 transition-all uppercase text-[10px] font-bold tracking-widest"
            ...
            {notifications.map((notif) => {
              ...
              return (
                <div
                  ...
                  className={`group relative flex items-start gap-3 p-4 transition-all duration-300 cursor-pointer ${!notif.read ? 'bg-orange-50/30' : 'hover:bg-slate-50'
                    }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {!notif.read && (
                        <span className="h-2 w-2 rounded-full bg-orange-400 shadow-sm shadow-orange-200 animate-pulse" />
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {notif.link && (
                        <a
                          ...
                          className="flex items-center gap-1 text-[9px] font-bold text-orange-400 hover:text-orange-500 uppercase tracking-widest"
                        >
