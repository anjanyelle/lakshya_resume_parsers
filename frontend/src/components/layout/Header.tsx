    ...
    <div className="flex items-center justify-between px-4 md:px-8 py-4 border-b border-slate-100 bg-white sticky top-0 z-30 h-20">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <h1 className="text-xl md:text-2xl font-black text-slate-700 tracking-tight truncate leading-none">
            {pageInfo.title}
          </h1>
        </div>
      </div>
      ...
      <div className="flex items-center gap-8 md:gap-12 ml-auto">
        ...
        <div className="relative hidden xl:block" ref={searchRef}>
           <div 
             className={`flex items-center gap-4 pl-6 pr-2 py-3 rounded-2xl transition-all duration-700 border backdrop-blur-xl ${
               isSearchOpen ? 'bg-white/90 border-violet-300 shadow-2xl shadow-violet-200/30 w-[480px]' : 'bg-slate-50/50 border-slate-100 w-80 hover:border-slate-200 hover:bg-white hover:shadow-lg hover:shadow-slate-200/40'
           ...
           >
              ...
              <input 
                ...
                className="flex-1 bg-transparent border-none text-[11px] font-black text-slate-700 focus:outline-none placeholder:text-slate-400/50 placeholder:font-black placeholder:tracking-[0.2em] uppercase"
                value={searchQuery}
                onChange={(e) => {
        ...
        <div className="flex items-center gap-4">
           ...
           <div className="relative" ref={notificationRef}>
              <button 
                ...
                className={`relative p-2.5 rounded-xl transition-all duration-300 group ${
                   isNotificationsOpen ? 'bg-violet-50 text-violet-600 shadow-inner' : 'text-slate-400 hover:bg-slate-50'
                }`}
              >
           ...
           <div className="relative" ref={profileRef}>
              <div 
                ...
                className="flex items-center gap-2 md:gap-3 cursor-pointer group p-1 rounded-2xl hover:bg-slate-50 transition-all duration-300"
              >
                <div className={`flex h-10 w-10 items-center justify-center rounded-2xl transition-all duration-500 group-hover:scale-105 ${config.boxClass}`}>
                  <User className={`h-5 w-5 ${config.iconClass}`} />
                </div>
                <div className="hidden sm:block">
                  <h4 className="text-[14px] font-black text-slate-700 leading-none uppercase tracking-tighter">
                    {config.label}
                  </h4>
                </div>
                <ChevronDown className={`h-4 w-4 text-slate-400 transition-all duration-300 ${isProfileOpen ? 'rotate-180 text-violet-500' : 'group-hover:translate-y-0.5'}`} />
              </div>
              ...
              {isProfileOpen && (
                 <div className="absolute right-0 mt-3 w-48 overflow-hidden rounded-2xl bg-white p-1.5 shadow-2xl ring-1 ring-slate-100 animate-in fade-in zoom-in-95 slide-in-from-top-2">
                    <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-black text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all uppercase tracking-widest">
