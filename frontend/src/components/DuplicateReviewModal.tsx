...
import { AlertTriangle, User, Mail, Phone, Briefcase, GraduationCap, Calendar, Info, X } from "lucide-react";
  ...
  const fetchCandidates = async () => {
    ...
    try {
      const [c1, c2] = await Promise.all([
        api.get(`/api/candidates/${duplicateGroup.candidate_1_id}`),
        api.get(`/api/candidates/${duplicateGroup.candidate_2_id}`)
      ]);
