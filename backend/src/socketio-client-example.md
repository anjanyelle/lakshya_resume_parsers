...
socket.on('parsing:complete', (data) => {
  ...
  showSuccessNotification(`Resume parsing completed for candidate ${data.candidateId}`)
...
function ParsingStatus({ candidateId, progress, completed, failed }) {
  ...
  if (isFailed) {
    ...
    return <span className="badge badge-danger" title={failedItem.error}>❌ Failed</span>
  }
  
  if (inProgress) {
    return <span className="badge badge-warning">🔄 Parsing ({inProgress.progress}%)</span>
  }
