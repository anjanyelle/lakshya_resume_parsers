  ...
  try {
    ...
    try {
      ...
      const updateQuery = `
        ...
        RETURNING id, company_name, industry, address, city, country, owner_user_id, is_archived, tenant_id, created_at, updated_at
      `;
...
export const archiveClient = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    try {
      ...
      // Archive the client
      const result = await client.query(
        "UPDATE clients SET is_archived = true, updated_at = NOW() WHERE id = $1 RETURNING *",
        [clientId]
      );
  ...
  try {
    ...
    try {
      ...
      const updateQuery = `
        ...
        SET ${updateFields.join(', ')}
        WHERE id = $${updateValues.length}
        RETURNING id, client_id, contact_name, designation, email, phone, is_primary, created_at, updated_at
      `;
