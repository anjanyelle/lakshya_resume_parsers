      ...
      const query = `
        SELECT 
          ...
          CASE 
            ...
            WHEN j.min_experience_years IS NOT NULL THEN j.min_experience_years || '+ years'
            WHEN j.max_experience_years IS NOT NULL THEN 'Up to ' || j.max_experience_years || ' years'
            ELSE 'Not specified'
...
export const assignRecruiterToJob = async (
  ...
  try {
    ...
    try {
      ...
      // Log activity
      await client.query(
        `INSERT INTO activity_log (id, activity_type, related_id, user_id, created_at)
         VALUES ($1, $2, $3, $4, NOW())`,
        [crypto.randomUUID(), 'recruiter_assigned', jobId, userId]
      );
