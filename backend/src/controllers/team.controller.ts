      ...
      const query = `
        ...
        activity_stats AS (
          SELECT 
            ...
            COUNT(DISTINCT CASE WHEN al.activity_type = 'submission_created' THEN al.id END) as submissions_created,
            COUNT(DISTINCT CASE WHEN al.activity_type = 'submission_reviewed' THEN al.id END) as submissions_reviewed,
            COUNT(DISTINCT CASE WHEN al.activity_type = 'interview_scheduled' THEN al.id END) as interviews_scheduled,
            COUNT(DISTINCT CASE WHEN al.activity_type = 'candidate_created' THEN al.id END) as candidates_created,
            COUNT(DISTINCT CASE WHEN al.activity_type = 'job_created' THEN al.id END) as jobs_created
          FROM team_recruiters tr
          LEFT JOIN activity_log al ON tr.id = al.user_id AND al.tenant_id = tr.id
          WHERE (al.tenant_id = tr.id OR al.tenant_id IS NULL)
            ${dateFilter}
          GROUP BY tr.id
        )
        SELECT 
