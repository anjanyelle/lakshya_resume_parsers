...
export const getAllCandidates = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const client = await getClient()
    try {
      const { candidates, total } = await CandidateModel.findAll(client, page, limit, search)
...
export const updateCandidate = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const client = await getClient()
    try {
      const candidate = await CandidateModel.update(client, candidateId, updates)
...
export const getCandidateParsingStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    const candidateId = Array.isArray(id) ? id[0] : id
    ...
    const client = await getClient()
    try {
      const parsingJob = await CandidateModel.getParsingStatus(client, candidateId)
