...
const fileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  ...
  if (allowedMimeTypes.includes(file.mimetype)) {
    ...
    const error = new Error(`Invalid file type: ${file.mimetype}. Only PDF, DOCX, and TXT files are allowed.`)
    cb(error as any)
  }
}
...
export const validateUploadedFile = (file: Express.Multer.File) => {
  ...
  if (file.size > maxSize) {
    throw new Error(`File size exceeds maximum allowed size of ${process.env.MAX_FILE_SIZE_MB || '10'}MB`)
  }
  ...
  if (!allowedMimeTypes.includes(file.mimetype)) {
    throw new Error(`Invalid file type: ${file.mimetype}. Only PDF, DOCX, and TXT files are allowed.`)
  }
...
export const addFileInfo = (req: Request, res: Response, next: NextFunction) => {
  if (req.file) {
