...
SECTION_ALIASES: dict[str, list[str]] = {
    ...
    "summary": [
        ...
        "summary of qualifications","ABOUT ME","OBJECTIVE","CAREER OBJECTIVE","OVERVIEW",
    ...
    "publications": [
        # English
        "publications", "publications & presentations", "research publications", "research", "research & publications", "papers", "research papers",
        ...
        "RESEARCH & PUBLICATIONS","SPEAKING ENGAGEMENTS","THOUGHT LEADERSHIP",
...
class SectionParser:
    ...
    def _match_header_line(
        self, line: str, blank_surrounded: bool, _recursion_depth: int = 0
    ) -> tuple[str, float, str, str] | None:
        if not line or _recursion_depth > 2:  # Allow depth 2 for balanced accuracy
            return None
        stripped = line.strip()
        
        # BALANCED PATTERNS - 6 key patterns for accuracy + speed
        # PATTERN 1: Markdown ## heading
        if stripped.startswith("## "):
            candidate = stripped[3:].strip()
            if candidate and not self._looks_like_bullet(candidate):
                match = self._match_header_line(candidate, blank_surrounded, _recursion_depth + 1)
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.04), "markdown_heading", evidence or candidate
        ...
        if stripped.startswith("# "):
            ...
            if candidate and not self._looks_like_bullet(candidate):
                ...
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.03), "markdown_heading", evidence or candidate
        
        # PATTERN 3: ALL CAPS heading (important for resumes)
        if re.match(r'^[A-Z][A-Z\s&/,()]+$', stripped):
            ...
            if candidate and not self._looks_like_bullet(candidate):
                ...
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.06), "all_caps", evidence or candidate
        
        # PATTERN 4: Heading with colon (very common in resumes)
        if stripped.endswith(":"):
            ...
            if candidate and not self._looks_like_bullet(candidate):
                ...
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.05), "colon_heading", evidence or candidate
        
        # PATTERN 5: Title Case heading (professional format)
        if re.match(r'^[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*$', stripped):
            ...
            if candidate and not self._looks_like_bullet(candidate):
                ...
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.04), "title_case", evidence or candidate
        
        # PATTERN 6: Bold markdown (common in modern resumes)
        if stripped.startswith("**") and stripped.endswith("**"):
            ...
            if candidate and not self._looks_like_bullet(candidate):
                ...
                if match is not None:
                    key, confidence, _, evidence = match
                    return key, min(1.0, float(confidence) + 0.03), "bold_markdown", evidence or candidate
        
        # Skip complex patterns for speed
        if self._looks_like_bullet(line):
            return None
        
        # Use basic matching as fallback
        return self._basic_header_match(stripped)
    
    def _basic_header_match(self, candidate: str) -> tuple[str, float, str, str] | None:
        """Basic header matching for balanced approach"""
        if not candidate:
            return None
        ...
        for key, aliases in SECTION_ALIASES.items():
            for alias in aliases:
                if lowered == alias.lower():
                    return key, 0.75, "direct_match", candidate
