...
class DatasetLoader:
    ...
    def __init__(self, cache_dir: Optional[str] = None, cache_ttl_hours: int = 24):
        ...
        self.dataset_configs = {
            'companies': {
                'files': [
                    '../../../data/external/clients.csv',  # Use available CSV files
                    '../../../data/external/clients.csv'
                ],
                'required_columns': ['company', 'normalized_company']  # Simplified columns
            },
            'job_titles': {
    ...
    def _get_project_root(self) -> str:
        ...
        base_dir = os.path.dirname(__file__)
        # Go up from utils/parser/app/services/backend to project root
