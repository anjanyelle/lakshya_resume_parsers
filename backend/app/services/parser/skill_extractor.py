...
# Load skills reference dataset FIRST (BALANCED APPROACH)
try:
    skills_df = pd.read_csv('data/external/skills.csv')
    # Use 200 skills for balance between speed and accuracy
    skills_df = skills_df.head(200)
    ...
    print("Using BALANCED approach: 200 skills loaded")
except:
    SKILLS_REFERENCE = set()
    SKILLS_CATEGORIES = {}

# Load trained skills NER model SECOND (BALANCED APPROACH)
try:
    skills_nlp = spacy.load("en_core_web_sm", disable=["parser", "lemmatizer"])  # Use standard model, disable heavy components
    # Add SKILL entity if not present
    if "SKILL" not in skills_nlp.pipe_labels["ner"]:
        ruler = skills_nlp.add_pipe("entity_ruler", before="ner")
        patterns = [
            {"label": "SKILL", "pattern": [{"LOWER": {"IN": list(SKILLS_REFERENCE)[:200]}}]}  # Use 200 skills for balance
        ]
        ruler.add_patterns(patterns)
    SKILLS_MODEL_LOADED = True
    print("Using BALANCED spaCy model with 200 skills patterns")
except Exception as e:
    SKILLS_MODEL_LOADED = False
    print(f"Warning: Could not load balanced NLP model: {e}")
...
class SkillExtractor:
    ...
    def extract_all(
        ...
        *,
        ...
        # Use BALANCED ML processing for speed + accuracy
        if SKILLS_MODEL_LOADED and raw_text:
            try:
                # Limit text to first 1500 characters for balanced speed
                limited_text = raw_text[:1500] if len(raw_text) > 1500 else raw_text
                
                doc = skills_nlp(limited_text)
                ml_skills = []
                for ent in doc.ents:
                    if ent.label_ == "SKILL":
                        ...
                        if skill_lower in SKILLS_REFERENCE:
                            ml_skills.append(SkillMatch(
                                name=skill_name,
                                normalized_name=skill_name,
                                category=SKILLS_CATEGORIES.get(skill_lower, "Technical Skills"),
                                confidence=0.85,  # Slightly lower confidence for speed
                                years_experience=None,
                                proficiency=None,
                                source="ML_BALANCED",
                                version=None
                            ))
                # Add ML skills to enriched results
                if ml_skills:
                    enriched.extend(ml_skills)
                    # Normalize to remove duplicates
                    enriched = self.normalize_skills(enriched)
            except Exception as e:
                logger.warning(f"Balanced ML skill extraction failed: {e}")
