...
def _parse_date_str(value: str | None) -> datetime.date | None:
    if not value:
        return None
    raw = str(value).strip()
    if not raw:
...
def task_extract_work_experience_details(self, job_id: str) -> str:  # noqa: ANN001
    ...
    try:
        ...
        job = _load_job(job_id)
        parsed = job.parsed_data or {}
        # Remove early return to allow enhanced pipeline to run
        ...
        # Use enhanced ML pipeline following the specified architecture
        try:
            from app.services.enhanced_pipeline import EnhancedResumePipeline
            enhanced_pipeline = EnhancedResumePipeline()
            
            # Get full resume text for enhanced processing
            full_resume_text = job.raw_text or ""
            
            print(f"🚀 ENHANCED ML PIPELINE ACTIVATED!")
            print(f"📄 Resume text length: {len(full_resume_text)} characters")
            
            # Enhanced ML Pipeline with all components
            complete_json = enhanced_pipeline.parse_resume_complete(full_resume_text, use_ml=True)
            
            print(f"✅ ENHANCED PIPELINE SUCCESS!")
            print(f"📊 Sections generated: {list(complete_json.keys())}")
            print(f"💼 Work entries: {len(complete_json.get('work', []))}")
            
            # Extract work experience from complete JSON for compatibility
            payload = complete_json.get('work', [])
            
        except ImportError as e:
            # Fallback to existing LLM service if enhanced pipeline not available
            print(f"❌ ENHANCED PIPELINE IMPORT ERROR: {e}")
            print("🔄 FALLING BACK TO OLD PIPELINE")
            llm = LLMParsingService()
        ...
        # Store work experience and complete JSON data
        if payload:
            parsed_update = {
                "work_experience_llm": payload
            }
            
            # Add complete JSON if available
            if complete_json:
                parsed_update["complete_resume_json"] = complete_json
                print(f"💾 STORING COMPLETE JSON with {len(complete_json)} sections")
            else:
                print("❌ NO COMPLETE JSON TO STORE")
            
            _update_job(
                job_id,
                last_stage="extract_work_experience_details",
                parsed_data=_merge_parsed(job, parsed_update),
            )
...
def task_parse_work_experience(self, job_id: str) -> str:  # noqa: ANN001
    ...
    try:
        ...
        # Use enhanced ML pipeline with custom trained models
        try:
            ...
            print(f"🚀 FINAL ENHANCED PIPELINE ACTIVATED!")
            print(f"📄 Resume text length: {len(full_resume_text)} characters")
            print(f"🤖 Using Final Enhanced Pipeline with robust parsing")
            
            # Enhanced ML Pipeline with custom trained models
            complete_json = enhanced_pipeline.parse_resume_complete(full_resume_text, use_ml=True)
            
            print(f"✅ FINAL ENHANCED PIPELINE SUCCESS!")
            print(f"📊 Sections generated: {list(complete_json.keys())}")
            print(f"💼 Work entries: {len(complete_json.get('work', []))}")
            ...
            # Extract work experience from complete JSON for compatibility
            work_entries = complete_json.get('work', [])
            
        except ImportError as e:
            # Fallback to existing Hybrid parser if enhanced pipeline not available
            print(f"❌ ENHANCED PIPELINE IMPORT ERROR: {e}")
            print("🔄 FALLING BACK TO HYBRID PARSER")
            try:
            ...
            work_entries = []
            complete_json = None
        # Initialize payload with enhanced pipeline results if available
        payload: list[dict[str, object]] = []
        primary_jobs = []  # Initialize primary_jobs to avoid NameError
        excerpt = ""  # Initialize excerpt to avoid NameError
        chosen_experience_text = ""  # Initialize chosen_experience_text to avoid UnboundLocalError
        raw_text = job.raw_text or ""  # Initialize raw_text to avoid UnboundLocalError
        method_used = "Unknown"  # Initialize method_used to avoid UnboundLocalError
        method_confidence = 0.0  # Initialize method_confidence to avoid UnboundLocalError
        parser = None  # Initialize parser to avoid UnboundLocalError
        source_format = "auto"  # Initialize source_format to avoid UnboundLocalError
        
        if complete_json and work_entries:
            # Validate enhanced pipeline results
            valid_entries = 0
            for job_entry in work_entries:
                if (job_entry.get("company") and job_entry.get("title") and 
                    not job_entry.get("company", "").startswith("- ")):
                    valid_entries += 1
            
            if valid_entries >= len(work_entries) * 0.5:  # At least 50% valid entries
                # Use enhanced pipeline results
                print(f"🎯 USING ENHANCED PIPELINE RESULTS: {len(work_entries)} work entries")
                for job_entry in work_entries:
                    payload.append({
                        "company": job_entry.get("company"),
                        "title": job_entry.get("title"),
                        "start_date": job_entry.get("startDate"),
                        "end_date": job_entry.get("endDate"),
                        "is_current": not job_entry.get("endDate") or job_entry.get("is_current", False),
                        "location": job_entry.get("location"),
                        "description": job_entry.get("description"),
                        "bullets": job_entry.get("bullets", []),
                        "client": job_entry.get("client"),
                    })
                
                # Convert enhanced pipeline results to JobEntry format for compatibility
                from app.services.parser.work_experience_parser import JobEntry
                from datetime import date
                primary_jobs = []
                for job_entry in work_entries:
                    job_entry_obj = JobEntry(
                        company=job_entry.get("company", ""),
                        title=job_entry.get("title", ""),
                        start_date=_parse_date_str(job_entry.get("startDate")) if job_entry.get("startDate") else None,
                        end_date=_parse_date_str(job_entry.get("endDate")) if job_entry.get("endDate") else None,
                        is_current=not job_entry.get("endDate") or job_entry.get("is_current", False),
                        location=job_entry.get("location", ""),
                        description=job_entry.get("description", ""),
                        bullets=job_entry.get("bullets", []),
                        duration_months=None,  # Will be calculated later if needed
                        client=job_entry.get("client"),
                        employment_type=None,  # Not available from enhanced pipeline
                        confidence=0.8,  # Default confidence for enhanced pipeline
                    )
                    primary_jobs.append(job_entry_obj)
                
                # Set method info for enhanced pipeline
                method_used = "Enhanced ML Pipeline"
                method_confidence = 0.9
                chosen_experience_text = job.raw_text or ""  # Set default for enhanced pipeline
            else:
                # Enhanced pipeline produced malformed data, fall back to traditional parsing
                print(f"❌ ENHANCED PIPELINE PRODUCED MALFORMED DATA (valid: {valid_entries}/{len(work_entries)})")
                print("🔄 FALLING BACK TO TRADITIONAL PARSING")
                work_entries = []
                complete_json = None
        else:
            # Fallback to traditional parsing
            print("🔄 USING TRADITIONAL PARSING FALLBACK")
            source_format = _source_format_from_path(
                getattr(job, "file_path", None) or getattr(job, "extracted_text_path", None)
            )
            
            # Initialize parser for traditional parsing
            try:
                from app.services.parser.hybrid_work_experience_parser import HybridWorkExperienceParser
                parser = HybridWorkExperienceParser()
                print("🔍🔍🔍 Using Hybrid Work Experience Parser (ML → Rule-based → LLM)")
        ...
        def _parse_deterministic(text: str) -> list[JobEntry]:
            ...
            # Traditional parsing logic only runs when enhanced pipeline is not available
                if raw_text.strip():
        ...
        # Calculate score for both enhanced pipeline and traditional parsing
        primary_score = score_work_experience_jobs(primary_jobs)
        logger.info(
            "DIAG primary_jobs count=%d first=%s method=%s confidence=%.2f",
            len(primary_jobs),
        ...
        if not payload:
            ...
            if parser is None:
                from app.services.parser.work_experience_parser import WorkExperienceParser
                parser = WorkExperienceParser()
                print("🔍🔍🔥 Initializing default parser (was None)")
        ...
        def _month_range_for(entry: dict[str, object]) -> tuple[int, int] | None:
            start_raw = entry.get("start_date") or entry.get("from") or entry.get("start")
            start = _parse_date_str(str(start_raw)) if start_raw else None
            if not start:
                return None
            end_raw = entry.get("end_date") or entry.get("to") or entry.get("end")
            is_current = bool(entry.get("is_current")) or _looks_like_present(str(end_raw) if end_raw else None)
            if is_current or _looks_like_present(str(end_raw) if end_raw else None):
                end = today
            elif end_raw:
                end = _parse_date_str(str(end_raw))
        ...
        for item in payload:
            if not isinstance(item, dict):
                continue
            entry = dict(item)
            start = _parse_date_str(str(entry.get("start_date"))) if entry.get("start_date") else None
            end_raw = entry.get("end_date")
            end = _parse_date_str(str(end_raw)) if end_raw else None
        ...
        def _sort_key(e: dict[str, object]) -> tuple[int, int]:
            ...
            if start is not None and hasattr(start, 'year') and hasattr(start, 'month'):
        ...
        # Add complete JSON if available from enhanced pipeline
        if complete_json:
            parsed_update["complete_resume_json"] = complete_json
            print(f"💾 STORING COMPLETE JSON with {len(complete_json)} sections")
        else:
